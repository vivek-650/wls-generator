...
export type CloudShaderProps = {
  ...
  /**
   * Stops the requestAnimationFrame draw loop entirely (freezing the last
   * frame) rather than just slowing it down — the animation is a
   * continuous WebGL redraw, cheap on a real GPU but real main-thread work
   * on every frame regardless. Pause it during anything latency-sensitive
   * happening in the same tab (e.g. a form submit that's about to
   * navigate) so it can't compete for the thread right when it matters.
   */
  paused?: boolean;
};
...
export const CloudShader = ({
  ...
  // Kicks the rAF loop again after a pause — set by the mount effect below,
  // called by the resume effect (a separate effect because it needs to
  // react to `paused` itself, not just read it off the ref).
  ...
  useEffect(() => {
    ...
    // Software rasterizers (SwiftShader, llvmpipe, and similar CPU-only GL
    // fallbacks — common in CI/headless browsers, some VMs/VDI sessions,
    // and a minority of real low-end/managed devices) make every GL draw
    // call expensive enough that a continuous animation loop can starve
    // the main thread badly — reproduced directly: it stalled an unrelated
    // page navigation for several seconds. A real GPU handles this shader
    // trivially, so gate the animation on detecting a software renderer
    // rather than degrading the experience for everyone.
    const debugInfo = gl.getExtension("WEBGL_debug_renderer_info");
    const rendererString = debugInfo
      ? String(gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL)).toLowerCase()
      : "";
    const isSoftwareRenderer = /swiftshader|llvmpipe|software|microsoft basic render/.test(rendererString);
    ...
    // Clouds drift slowly — 24fps reads as perfectly smooth for this kind
    // of motion, and capping the *draw* rate (not just the rAF callback
    // rate) is what actually matters: reproduced directly against
    // software-rendered WebGL (Chromium/SwiftShader, the same fallback
    // path CI runners, VMs, and some low-end/managed devices hit) — an
    // uncapped 60fps target falls behind there and the GPU process backs
    // up a queue of undrawn frames. That backlog doesn't drain just
    // because a later frame decides to pause — it stalled an unrelated
    // Next.js client-side navigation for several seconds in testing.
    // Bounding the draw rate up front is what prevents the backlog from
    ...
    const draw = (now: number) => {
      ...
      // Stop scheduling entirely while paused — a skipped GL draw still
      // costs a requestAnimationFrame callback every frame, which is real
      // main-thread contention exactly when something latency-sensitive
      // (e.g. a form submit that's about to navigate) needs the thread.
      // `resumeRef` re-arms the loop once `paused` goes false again.
      if (p.paused) return;
      ...
      // Under a software rasterizer, render this one frame (still a real
      // sky + clouds, just not animated) and stop — don't keep scheduling.
